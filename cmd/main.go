...
import (
	...
	// Import renderer packages for init() side effects.
...
const (
	enablePoolUniquenessWebhookEnv = "ASTRADNS_ENABLE_POOL_UNIQUENESS_WEBHOOK"
	defaultLeaderElectionID        = "60acac32.astradns.com"
)
...
func main() {
	...
	flag.StringVar(&leaderElectionID, "leader-election-id", defaultLeaderElectionID,
		"Leader election lease identifier shared by operator replicas.")
	...
	engineTypeName := os.Getenv("ASTRADNS_ENGINE_TYPE")
	if engineTypeName == "" {
		engineTypeName = string(engine.EngineUnbound)
	}
	engineType := engine.EngineType(engineTypeName)

	configGen := &operatorconfig.DefaultConfigGenerator{}
	configRenderer, err := typesengineconfig.NewRenderer(engineType)
	if err != nil {
		setupLog.Error(err, "Unable to create config renderer", "engineType", engineType)
		os.Exit(1)
	}

	if err = (&controllers.DNSUpstreamPoolReconciler{
